...
export default function SkillPostFeed({ editable = false }) {
  ...
  const handleAddComment = async (postId) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/api/v1/comment?postId=${postId}`,
        {
          userId: id,
  ...
  const handleDeleteComment = async (postId, commentId) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/v1/comment?postId=${postId}&userId=${id}&commentId=${commentId}`
      );
  ...
  const handleSaveEditedComment = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/api/v1/comment?postId=${editPostId}&userId=${id}&commentId=${editCommentId}`,
        {
          comment: editComment,
        }
      );
  ...
  const handleLike = async (postId) => {
    ...
    try {
      await axios.post(
        `http://localhost:8080/api/v1/like?postId=${postId}&userId=${userId}`
      );
      ...
      {posts?.map((post) => (
        <div key={post.postId} className="posts__card">
          ...
          <div className="posts__actions enhanced-actions">
            <button
              className={`posts__action-btn ${
                likedPosts[post.postId] ? "liked" : ""
              }`}
              onClick={() => handleLike(post.postId)}
            >
              <FaThumbsUp
                style={{ color: likedPosts[post.postId] ? "blue" : "inherit" }}
              />{" "}
          ...
          <div className="posts__comments styled-comments">
            {post.comments?.length > 0 ? (
                ...
                <div key={i} className="styled-comment">
                  ...
                  {c?.user.id === user.id && (
                    <div className="comment-actions">
                      <button
                        onClick={() =>
                          handleEditComment(post.postId, c.commentId, c.comment)
                        }
                      >
                        Edit
                      </button>
              ...
              <div className="posts__comment posts__comment--empty">
                No comments yet.
              </div>
            )}
